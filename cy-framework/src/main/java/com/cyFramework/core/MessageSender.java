...
public class MessageSender {
    ...
    /**
     * Envoie un message :
     * - Si l'acteur existe localement → FIFO
     * - Sinon → HTTP via Consul
     */
    public void envoyer(String serviceCible, Message message) {

        // Vérifie l'existence locale de l'acteur
        Acteur acteurLocal = ActorRegistry.trouver(message.getDestinataire());

        if (acteurLocal != null) {
            acteurLocal.getLogger().debug("Message local stocké pour traitement FIFO");
            acteurLocal.ajouterMessage(message); // 🔥 FINALLY CORRECT
            return;
        }
